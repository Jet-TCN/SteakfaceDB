...
def get_recipes_and_ingredients():
    recipes = steakfacedb.get_all_recipes()
    ingredients = steakfacedb.get_all_ingredients()
...
def index(pagetype: str = "ADD_RECIPE"):
    ...
    for recipe in recipes:
        ...
        for id in id_list:
            print(id)
            name_list.append(steakfacedb.get_ingredient_name_from_id(id))
    ...
    for ingredient in ingredients:
        ...
        for id in id_list:
            name_list.append(steakfacedb.get_recipe_name_from_id(id))
...
def add_recipe():
    ...
    course_type = steakfacedb.CourseEnum.BREAKFAST

    if course_name.upper() not in ["BREAKFAST", "LUNCH", "DINNER"]:
        return render_template('index.html', recipes=recipes, show_alert=True, error_message="BADCOURSE",
                               pagetype="ADD_RECIPE",
                               ingredients=ingredients)
    elif course_name.upper() in "LUNCH":
        course_type = steakfacedb.CourseEnum.LUNCH
    elif course_name.upper() in "DINNER":
        course_type = steakfacedb.CourseEnum.DINNER
    ...
    if sorted(ingredient_list, reverse=True)[0] > ingredient_limit:
        return render_template('index.html', recipes=recipes, show_alert=True, error_message="OUTOFRANGEINGREDIENTID",
                               pagetype="ADD_RECIPE",
                               ingredients=ingredients)

    # ingredient_id_list = [steakfacedb.get_ingredient_id(ingredient) for ingredient in ingredient_list]
    try:
        ingredient_id_list = [int(ingredient) for ingredient in ingredient_list]
        steakfacedb.insert_recipe(recipe_name, course_type, ingredient_id_list)
    except ValueError:
        return render_template('index.html', recipes=recipes, show_alert=True, error_message="INVALIDINGREDIENT",
                               pagetype="ADD_RECIPE")
...
def del_recipe():
    ...
    try:
        ...
        return render_template('index.html', recipes=recipes, show_alert=True, error_message="INVALIDRECIPEID",
                               pagetype="DEL_RECIPE")

    if steakfacedb.get_current_recipe_id() < recipe_id or recipe_id < steakfacedb.get_smallest_recipe_id():
        return render_template('index.html', recipes=recipes, show_alert=True, error_message="OUTOFRANGERECIPEID",
                               pagetype="DEL_RECIPE")

    steakfacedb.delete_recipe_by_id(recipe_id)
    return redirect(request.referrer)
...
def add_ingred():
    ...
    steakfacedb.insert_ingredient(ingredient)
    return redirect(request.referrer)
...
def del_ingred_index():
    ...
    try:
        ...
        return render_template('index.html', ingredients=ingredients, show_alert=True, pagetype="DEL_INGRED",
                               error_message="TOOHIGHINGREDIENT")

    steakfacedb.delete_ingredient_by_id(ingredient)
    return redirect(request.referrer)
